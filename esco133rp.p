...
Assign c-arquivo = session:Temp-dir + "ESCO133" + replace(String(Today,"99/99/9999"),"/","") + Replace(String(Time,"hh:mm:ss"),":","") + ".csv".
    ...
    For Each pedido-compr Where
        ...
        If Avail mgcolomb.cotacao Then Do:
           ...
           If Avail mgcolomb.cot-ordem Then Do:
               ...
               ASSIGN tt-pedido.cod-justif  = if cot-ordem.cod-justif = 0 then 1 else cot-ordem.cod-justif
                      tt-pedido.desc-justif = c-descr[if cot-ordem.cod-justif = 0 then 1 else cot-ordem.cod-justif]
                      tt-pedido.nr-processo = ordem-compra.nr-processo. 
